...
namespace Base
{
	...
	// calculate point in motitor space( Norm Device Coord )
	//
	inline float PositionToNDC( float sizeDevice, float pos )
	{
		return pos / sizeDevice * 2.0f - 1.0f;
	}

	// calculate dimension in motitor space( Norm Device Coord )
	//
	inline float DimensionToNDC( float sizeDevice, float size )
	{
		return size / sizeDevice * 2.0f;
	}

	// calculate point in space from NDC
	// pos in monitor [-1.0, 1.0f]
	//
	inline float PositionFromNDC( float sizeDevice, float pos )
	{
		return ( sizeDevice * ( pos + 1.0f ) ) / 2.0f;
	}

	// calculate dimension from Norm Device Coord
	// size in monitor [0.0, 2.0f]
	//
	inline float DimensionFromNDC( float sizeDevice, float size )
	{
...
#define BINARY_OPERATION_TO_ENUM_CLASS( type ) \
	inline bool operator&( const type& x, const type& y )\
	{\
		return ( ( int )x & ( int )y ) ? true : false;\
	}